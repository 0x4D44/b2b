use crate::{cli::{Cli, RoleKind}, logging, util};
use anyhow::{Context, Result};
use std::{collections::HashSet, path::PathBuf, process::{Child, Command, Stdio}, sync::mpsc, thread, time::{Duration, Instant}};
...
fn load_plan(path: &PathBuf) -> Result<PlanTopology> {
    let s = std::fs::read_to_string(path).with_context(|| format!("reading plan: {}", path.display()))?;
...
fn spawn_all(topo: &PlanTopology, args: &Cli) -> Result<Vec<(RoleKind, Child)>> {
    let mut procs: Vec<(RoleKind, Child)> = Vec::new();
    // sink
    if let Some(bind) = topo.sink_bind.as_deref() {
        let mut extra = vec!["--sip-bind".to_string(), bind.to_string()];
        if let Some(cmd) = topo.sink_aplay_cmd.as_deref() { extra.push("--aplay-cmd".into()); extra.push(cmd.into()); }
        procs.push(spawn_role(RoleKind::Sink, &extra, args)?);
    }
    // mixer
    if let (Some(bind), Some(target)) = (topo.mixer_bind.as_deref(), topo.mixer_target.as_deref()) {
        let mut extra: Vec<String> = vec!["--sip-bind".into(), bind.into(), "--target".into(), target.into()];
        if let Some(seq) = topo.mixer_dtmf_seq.as_deref() { extra.push("--dtmf-seq".into()); extra.push(seq.into()); }
        procs.push(spawn_role(RoleKind::Mixer, &extra, args)?);
    }
    // source
    if let Some(target) = topo.source_target.as_deref() {
        let mut extra: Vec<String> = vec!["--target".into(), target.into()];
    ...
    // Pipe child outputs
    for (role, child) in procs.iter_mut() {
        let (out, err) = util::child_pipes(child);
        let tag = logging::role_tag(match role { RoleKind::Sink=>"sink", RoleKind::Source=>"source", RoleKind::Mixer=>"mixer", RoleKind::Orchestrator=>"orchestrator" });
        if let Some(o) = out { util::spawn_reader_thread(o, tag.clone(), |tag, line| logging::println_tag(&tag, line)); }
        if let Some(e) = err { util::spawn_reader_thread(e, tag.clone(), |tag, line| logging::println_tag(&tag, line)); }
    }
    Ok(procs)
}

fn spawn_role(role: RoleKind, extra: &[String], args: &Cli) -> Result<(RoleKind, std::process::Child)> {
    let exe = exe()?;
    let mut cmd = Command::new(exe);
    cmd.arg("--role").arg(role_str(role));
    // pass consistent output options
    cmd.args(["--log-format", match args.log_format { crate::cli::LogFormat::Text => "text", crate::cli::LogFormat::Json => "json" }]);
    cmd.args(["--color", match args.color { crate::cli::ColorChoice::Auto=>"auto", crate::cli::ColorChoice::Always=>"always", crate::cli::ColorChoice::Never=>"never" }]);
    // Ensure children emit raw lines; we add timestamp + [ROLE] here.
    cmd.env("B2B_CHILD_RAW", "1");
    cmd.args(extra);
    cmd.stdout(Stdio::piped()).stderr(Stdio::piped());
    let child = cmd.spawn().with_context(|| format!("spawning role {}", role_str(role)))?;
    Ok((role, child))
}

fn role_str(r: RoleKind) -> &'static str { match r { RoleKind::Orchestrator=>"orchestrator", RoleKind::Source=>"source", RoleKind::Sink=>"sink", RoleKind::Mixer=>"mixer" } }

fn print_plan_cmds(topo: &PlanTopology, _args: &Cli) {
    let exe = std::env::current_exe().ok().and_then(|p| p.file_name().map(|n| n.to_string_lossy().to_string())).unwrap_or_else(|| "b2b".into());
    let orch = logging::role_tag("orchestrator");
    if let Some(bind) = topo.sink_bind.as_deref() {
        let mut cmd = format!("dry-run: {exe} --role sink --sip-bind {bind}");
        if let Some(ap) = topo.sink_aplay_cmd.as_deref() { cmd.push_str(&format!(" --aplay-cmd \"{}\"", ap)); }
        logging::println_tag(&orch, &cmd);
    }
    if let (Some(bind), Some(target)) = (topo.mixer_bind.as_deref(), topo.mixer_target.as_deref()) {
        logging::println_tag(&orch, &format!("dry-run: {exe} --role mixer --sip-bind {bind} --target {target}"));
    }
    if let Some(target) = topo.source_target.as_deref() {
        let af = topo.source_audio_file.as_ref().map(|p| format!(" --audio-file {}", p.display())).unwrap_or_default();
        logging::println_tag(&orch, &format!("dry-run: {exe} --role source --target {target}{af}"));
    }
}
...
fn shutdown_children(children: &mut [(RoleKind, Child)], grace_ms: u64, kill_ms: u64) {
    // Send SIGTERM first
    for (_, ch) in children.iter_mut() { let id = ch.id(); unsafe { libc::kill(id as i32, libc::SIGTERM); } }
    let deadline = Instant::now() + Duration::from_millis(grace_ms);
    while Instant::now() < deadline {
        let mut all_done = true;
        for (_, ch) in children.iter_mut() {
            match ch.try_wait() { Ok(Some(_)) => {}, Ok(None) => { all_done = false; }, Err(_) => {} }
        }
        if all_done { return; }
    ...
    for (_, ch) in children.iter_mut() { let id = ch.id(); unsafe { libc::kill(id as i32, libc::SIGKILL); } }
...
fn monitor_or_ctrlc(tag: &str, children: &mut Vec<(RoleKind, Child)>, grace_ms: u64, kill_ms: u64) {
    // Listen for Ctrl-C
    let (ctx, crx) = mpsc::channel::<()>();
    let _ = ctrlc::set_handler(move || { let _ = ctx.send(()); });
...
fn looks_preformatted(s: &str) -> bool {
    // Skip ANSI CSI sequences at the start like \x1b[...m
...
fn pipe_child_output(role: &RoleKind, child: &mut Child, tx: mpsc::Sender<String>) {
    ...
    if let Some(o) = out {
        util::spawn_reader_thread(o, tag.clone(), move |_tag, line| {
            if let Some(role) = parse_ready_role(line) { let _ = txo.send(role.to_string()); }
    ...
    if let Some(e) = err {
        ...
        util::spawn_reader_thread(e, tag.clone(), move |_tag, line| {
            if let Some(role) = parse_ready_role(line) { let _ = tx.send(role.to_string()); }
...
fn parse_ready_role(line: &str) -> Option<&str> {
    ...
    for tok in line.split_whitespace() {
        if let Some(val) = tok.strip_prefix("role=") { return Some(val); }
    }
    None
}
...
fn wait_for_ready(expected: &HashSet<&'static str>, ready_ms: u64, rx: &mpsc::Receiver<String>) {
    ...
    while Instant::now() < deadline {
        if let Ok(role) = rx.recv_timeout(Duration::from_millis(100)) {
            seen.insert(role);
            if expected.iter().all(|r| seen.contains(*r)) { break; }
        }
    }
}
