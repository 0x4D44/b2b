...
pub fn run(args: &Cli) -> Result<()> {
    ...
    let target = args.target.as_deref().context("mixer requires --target (sink)")?;
    ...
    unsafe { std::env::set_var("BRS_CONF_BUF", preload); }
    ...
    // Background: log key mixer events for both legs
    {
        ...
        std::thread::spawn(move || {
            while let Ok(ev) = rx_ev.recv() {
                ...
                if kind.contains("CALL_REMOTE_SDP") { logging::println_tag(&tag, "bevent: CALL_REMOTE_SDP (offer)"); }
                else if kind.contains("CALL_LOCAL_SDP") { logging::println_tag(&tag, "bevent: CALL_LOCAL_SDP (answer)"); }
                else if kind.contains("CALL_INCOMING") { logging::println_tag(&tag, &format!("bevent: CALL_INCOMING ({})", text)); }
                else if kind.contains("CALL_PROGRESS") { logging::println_tag(&tag, "bevent: CALL_PROGRESS"); }
                else if kind.contains("CALL_RTPESTAB") { logging::println_tag(&tag, "bevent: CALL_RTPESTAB"); }
                else if kind.contains("CALL_ESTABLISHED") { logging::println_tag(&tag, &format!("bevent: CALL_ESTABLISHED ({})", text)); }
                else if kind.contains("CALL_CLOSED") { logging::println_tag(&tag, &format!("bevent: CALL_CLOSED ({})", text)); }
            }
        });
    }
