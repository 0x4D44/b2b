...
pub fn decode_mp3_to_pcm_8k<P: AsRef<Path>>(path: P) -> Result<PcmFrames> {
    let file = File::open(&path).with_context(|| format!("open audio file: {}", path.as_ref().display()))?;
    ...
    if src_rate == dst_rate { return Ok(PcmFrames { data: pcm, sample_rate: 8000 }); }
    ...
    for i in 0..out_len {
        ...
        let v = a + (b - a) * frac;
        out[i] = v.round().clamp(i16::MIN as f64, i16::MAX as f64) as i16;
    }
    Ok(PcmFrames { data: out, sample_rate: 8000 })
}
